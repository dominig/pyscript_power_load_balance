...
def estimate_power_saving_mode(general_meter_reading):
    ...
    # when available power reduce, we need to be quick to increase the saving_power_mode
    ...
    # hysteresis is created when available power increase to reduce wear on electromagnetic relays.
    ...
    # realign power_save_mode to 0 when power is widely available whatever are hysteresis setting (likely miss configured)
    if heating_remaining_power/POWER_PER_RADIATOR > MAX_RADIATORS_ON and not TEST_MODE:
        log.debug(f"powersavingmode.py: saving_power_mode =0")
        if state.get(POWER_SAVING_MODE)!=0:
            log.info(f"powersavingmode.py: resetting power_saving_mode to 0 (off)")
            service.call('input_number','set_value',blocking=False, entity_id=POWER_SAVING_MODE)
